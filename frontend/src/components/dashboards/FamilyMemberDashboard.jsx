    ...
    <div className="space-y-8 fade-in">
      ...
      <div className="theme-card p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-extrabold text-slate-900">Household Shared Finance</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="theme-card p-6 space-y-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Scale className="w-5 h-5 text-[#DB5375]" />
                <h2 className="text-base font-extrabold text-slate-900">Instant Household Expense Splitter</h2>
              </div>
            ...
            <form onSubmit={handleAddSharedSplit} className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs items-end bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
              ...
              <div>
                <label className="block font-bold text-slate-800 mb-1">Total Bill Amount (₹)</label>
                <input
                  type="number"
                  step="0.01"
                  required
              ...
              <div>
                <label className="block font-bold text-slate-800 mb-1">Number of People</label>
                <input
                  type="number"
                  min="2"
                  max="10"
              ...
              <div className="sm:col-span-3 flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                <div className="text-xs text-slate-800">
                  Each member pays: <strong className="text-base font-black text-[#a82948]">₹{computedShare}</strong>
                </div>
                  ...
                  <div key={item.id} className="py-3 flex justify-between items-center text-xs hover:bg-slate-50 px-2 rounded-xl transition-colors">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 font-medium">
                        Paid by <strong className="text-slate-800">{item.paidBy}</strong> • {item.count} people split • {item.date}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-extrabold text-[#a82948]">₹{item.share.toLocaleString('en-IN')} / share</p>
                ...
                transactions.map((t) => (
                  <div key={t.id} className="py-3 flex justify-between items-center text-xs hover:bg-slate-50 px-2 rounded-xl transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-extrabold text-slate-900">{t.merchant || t.description}</p>
                        <p className="text-[11px] text-slate-500 font-medium">{t.category} • {t.txnDate?.substring(0, 10) || 'Recent'}</p>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="theme-card p-6 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-base font-extrabold text-slate-900">Upcoming Shared Bills</h3>
              <Link to="/bills" className="text-xs font-bold text-[#DB5375] hover:underline">All Bills</Link>
            </div>
              ...
              {upcomingBills.length === 0 ? (
                ...
                upcomingBills.map((b) => (
                  <div key={b.id} className="p-3.5 rounded-xl border border-slate-200/80 bg-white flex justify-between items-center text-xs shadow-xs hover:-translate-y-0.5 transition-all">
                    ...
                    <div className="text-right">
                      <p className="font-black text-[#a82948]">₹{Number(b.amount).toLocaleString('en-IN')}</p>
